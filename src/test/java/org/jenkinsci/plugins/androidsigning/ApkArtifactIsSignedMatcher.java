...
class ApkArtifactIsSignedMatcher extends BaseMatcher<BuildArtifact> {
    ...
    public boolean matches(Object item) {
        ...
        try {
            VirtualFile virtualSignedApk =
                    actual.build.getArtifactManager().root().child(actual.artifact.relativePath);
            FilePath signedApkPath = actual.build
                    .getWorkspace()
                    .createTempFile(actual.artifact.getFileName().replace(".apk", ""), ".apk");
